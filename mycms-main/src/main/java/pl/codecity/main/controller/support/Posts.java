...
import pl.codecity.main.utility.PostUtils;
...
public class Posts {
	...
	public Posts(IExpressionContext context, PostUtils postUtils, MyCmsProperties myCmsProperties) {
	...
	public String thumbnail(Post post) {
		if (post.getCover() != null) {
			return myCmsProperties.getMediaUrlPrefix() + post.getCover().getId();
		}
		else {
	...
	protected String parse(String html) {
		...
		for (Element element : elements) {
			...
			if (src.startsWith(myCmsProperties.getMediaUrlPrefix())) {
